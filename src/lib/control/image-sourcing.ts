/**
 * Image sourcing core (SPECS.md §I Part 10). One implementation, three
 * callers: the intake pipeline (sourceForIntake — every new tenant, D11),
 * the CLI (scripts/source-images.ts), and the seed's demo bootstrap.
 *
 * Moved here from scripts/lib in Session 1: the intake pipeline calls this
 * from the app, so it runs on the CONTROL role (curbside_control, explicit
 * RLS reach — migration 002), never the owner role the old script used.
 *
 ...
 *                  gallery page renders from images.credit — a url written
 *                  without its credit is a licensing bug, so both land in the
 *                  same UPDATE, always.
 *
 * Winners are stored through the blob seam (src/lib/blob.ts): Azure Blob in
 * production (a locally-sourced /uploads path only exists on the machine
 * that ran the CLI — the D11 miss this move fixes), local UPLOAD_DIR in dev.
 *
...
import { controlOne, controlQuery, revalidateTenant } from "./db";
import { saveSlotImage } from "../blob";
...
async function openverseSearchOnce(query: string, aspectParam: string | null): Promise<OpenverseResult[]> {
  ...
  const headers = { "User-Agent": "curbside-sites/1.0 (image sourcing)" };
  ...
  for (const photo of photos) {
    ...
    try {
      const img = await fetch(photo.downloadUrl, {
        ...
        headers: { "User-Agent": "curbside-sites/1.0 (image sourcing)" },
      });
      if (!img.ok) continue;
...
// ---------------------------------------------------------------------------
// Apply — write winners through the blob seam + the images manifest
// ---------------------------------------------------------------------------
...
export async function applyChoices(slug: string, choices: Map<string, number>, quiet = false) {
  ...
  for (const [slotId, n] of choices) {
    ...
    // Winners are named by their slot so the client can later drop in their
    // own photo under the same name with zero code edits (Part 10). Stored
    // through the blob seam: Azure Blob in production, local UPLOAD_DIR in
    // dev — never only on the machine that ran the sourcing (D11).
    const body = await readFile(join(dir, `${n}.jpg`));
    const publicUrl = await saveSlotImage(slug, `${slotId}.jpg`, body);

    // url and credit land in ONE update: Openverse results are CC BY /
    // CC BY-SA and the credit is what satisfies the licence.
    const updated = await controlQuery(
      `UPDATE images SET url = $3, credit = $4 WHERE tenant_id = $1 AND slot_id = $2 RETURNING slot_id`,
      [tenantId, slotId, publicUrl, candidate.credit]
    );
    if (updated.length === 0) throw new Error(`${slotId}: no images row for tenant '${slug}'`);
    meta.applied = n; // the contact sheet marks this pick on every rebuild
    await writeFile(metaPath, JSON.stringify(meta, null, 2));
    if (!quiet) console.log(`applied ${slotId} ← candidate #${n} → ${publicUrl}`);
  }

  await revalidateTenant(slug); // new images show without waiting out the ISR window
}
...
export async function writeSheetFromCache(slug: string): Promise<string | null> {
  const tenant = await controlOne<{ id: string }>("SELECT id FROM tenants WHERE slug = $1", [slug]);
  if (!tenant) throw new Error(`no tenant '${slug}'`);
  const slots = (await controlQuery(
...
// ---------------------------------------------------------------------------
// Full pipeline — intake, CLI, and seed all call this
// ---------------------------------------------------------------------------
...
export async function sourceForTenant(
  ...
  const t = await controlOne<{ id: string; business_name: string }>(
    "SELECT id, business_name FROM tenants WHERE slug = $1",
    [slug]
  );
  ...
  const bp = await controlOne("SELECT nap, tagline, about FROM business_profile WHERE tenant_id = $1", [t.id]);
  const serviceRows = await controlQuery("SELECT name FROM services WHERE tenant_id = $1 ORDER BY sort_order", [t.id]);
  const business: Business = {
    name: t.business_name,
    tagline: (bp?.tagline as string) ?? "",
    about: (bp?.about as string) ?? "",
    city: (bp?.nap?.city as string) ?? "",
    region: (bp?.nap?.region as string) ?? "",
  ...
  if (opts.auto) {
    console.log(
      "[images] AUTO MODE applied unreviewed picks — fine for a draft preview, NOT for go-live.\n" +
...
// ---------------------------------------------------------------------------
// The intake hook (D11): every new tenant sources its images, automatically
// ---------------------------------------------------------------------------

/**
 * Source images for a freshly-created intake tenant, then SURFACE the result:
 * any slot still without a URL raises a pending_actions item instead of
 * sitting silently pretty-looking-in-dev and empty in prod. Never throws —
 * the non-fatal degrade stands (no network → branded placeholders keep
 * serving) — but a degrade is always visible in the staff queue.
 *
 * SKIP_IMAGE_SOURCING=1 (CI, offline dev) skips the network work and goes
 * straight to the surfacing step, so even a skipped run leaves a queue item.
 */
export async function sourceForIntake(tenantId: string, slug: string): Promise<void> {
  try {
    if (process.env.SKIP_IMAGE_SOURCING === "1") {
      console.log(`[images] ${slug}: SKIP_IMAGE_SOURCING=1 — placeholders keep serving`);
    } else {
      await sourceForTenant(slug, { auto: true });
    }
  } catch (e) {
    console.error(`[images] intake sourcing failed for ${slug}: ${(e as Error).message}`);
  }

  try {
    const empty = await controlQuery<{ slot_id: string }>(
      `SELECT slot_id FROM images
        WHERE tenant_id = $1 AND purpose <> 'instagram' AND url IS NULL ORDER BY slot_id`,
      [tenantId]
    );
    if (empty.length === 0) return;
    // One open queue item per tenant, not one per retry.
    const open = await controlOne(
      `SELECT id FROM pending_actions
        WHERE tenant_id = $1 AND kind = 'source_images' AND status = 'pending'`,
      [tenantId]
    );
    if (open) return;
    await controlQuery(
      `INSERT INTO pending_actions (tenant_id, kind, reason, payload)
       VALUES ($1, 'source_images', $2, $3)`,
      [
        tenantId,
        `Image sourcing left ${empty.length} slot(s) on placeholders — run: npm run images:source ${slug} -- --auto`,
        JSON.stringify({ slug, empty_slots: empty.map((r) => r.slot_id) }),
      ]
    );
  } catch (e) {
    // Tenant may already be gone (e2e cleanup) — log, never crash the caller.
    console.error(`[images] could not record sourcing state for ${slug}: ${(e as Error).message}`);
  }
}
